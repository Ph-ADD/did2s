...
Researchers often want to fit a difference-in-differences (DiD) model in a regression setting. Typically, these have made use of the so-called _twoway fixed effects_ (TWFE) framework. For example, in a static setting:
...
where $D_{it}^k$ are lag/leads of treatment (k periods from initial treatment date) with $-L$ representing the start of the pre-treatment window and $K$ the end of the post-treatment window. Note that in the traditional TWFE framework at least one period must be dropped to avoid collinearity. Following convention, the first period before the treatment $(k = -1)$ is dropped in the above equation since it acts as the most natural reference period. 

> [!NOTE]
> Aside: Sometimes researchers use variants of this model where they bin or drop leads and lags. This might be required, for example, if there is completely untreated group. In that case  </aside>
...
> [!NOTE]
> Aside: This is the same logic as the new paper by [[Borusyak et. al. (2021)](https://sites.google.com/view/borusyak/research).
...
Then, let's estimate an event study DiD model. Note that relative year has a value of `Inf` for never treated, so I put this as a reference in the second stage formula.
